...
function App() {
  ...
  const handleKeyDown = useCallback(
    (e: KeyboardEvent) => {
      ...
      if (e.key === 'h' || e.key === 'H') {
        ...
        e.preventDefault();
        dispatch({ type: 'APPLY_MOVE', moveType: 'mod-3' });
  ...
  const handleLiveCellClick = (pos: CellPosition) => {
    ...
    if (currentMode === 'plus-3') {
      dispatch({ type: 'APPLY_MOVE', moveType: 'plus-3', position: pos });
    } else if (currentMode === 'horizontal-trimer') {
      dispatch({ type: 'APPLY_MOVE', moveType: 'horizontal-trimer', position: pos });
    } else if (currentMode === 'vertical-trimer') {
      dispatch({ type: 'APPLY_MOVE', moveType: 'vertical-trimer', position: pos });
    }
  };
  ...
  const handleHistoryJump = (index: number) => {
    ...
    // If jumping forward would truncate, just go to it
    // If jumping backward, just undo
  ...
  const handleExport = () => {
    ...
    if (navigator.clipboard) {
      navigator.clipboard.writeText(json).then(() => {
        ...
        // Use a brief toast-like approach: we'll set a message
  ...
  const downloadJson = (json: string) => {
    ...
    a.href = url;
    a.download = `trimer-state-${rows}x${cols}.json`;
    a.click();
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              Explore the four-fingerprint reachability test for trimer tilings
            </p>
          </div>
      ...
      <div className="flex flex-col lg:flex-row">
        ...
        <div className="lg:w-[55%] p-6 space-y-4">
          ...
          <Toolbar
            ...
            onMod3={() => dispatch({ type: 'APPLY_MOVE', moveType: 'mod-3' })}
            onFillPlus3={() => dispatch({ type: 'FILL_ALL_PLUS3' })}
            onReset={() => {
              if (history.length > 0) {
