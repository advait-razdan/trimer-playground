...
import { ProveWhatItDoes } from '../components/help/ProveWhatItDoes';
...
export function PolyominoRoute() {
  ...
  // Rules
  const [rules, setRules] = useState<RuleToggles>(() => loadRules('polyomino'));
  const handleRuleToggle = (key: keyof RuleToggles) => {
    setRules(prev => {
      const next = { ...prev, [key]: !prev[key] };
      saveRules('polyomino', next);
      return next;
    });
  };
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = `polyomino-state-${rows}x${cols}.json`;
    a.click();
        ...
        <div className="md:w-[45%] md:border-l border-gray-200 bg-white p-6 md:sticky md:top-0 md:h-screen md:overflow-y-auto">
          {shapeLocked && invariantBasis ? (
            <>
              ...
              {/* Prove button */}
              <div className="mb-4 flex items-center gap-1" data-tour="prove-button">
                {showTarget && gridB ? (
                  <>
                    <button
                      className="px-4 py-2 text-sm font-semibold bg-blue-600 text-white rounded hover:bg-blue-700 disabled:opacity-40 disabled:cursor-not-allowed"
                      onClick={() => setShowProve(true)}
                      title="Prove reachability of live grid to target B"
                    >
                      Prove reachability
                    </button>
                    <ProveWhatItDoes />
                  ...
                  <span
                    className="px-4 py-2 text-sm font-semibold bg-gray-300 text-gray-500 rounded cursor-not-allowed"
